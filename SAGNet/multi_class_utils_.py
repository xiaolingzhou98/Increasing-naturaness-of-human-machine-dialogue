...
LTP_DATA_DIR = '/home/nlp/Desktop/answer_generation/ltp_data_v3.4.0/'
_GO = '_GO'
...
def instance_augment(instances, question2targets):
    ...
    return end_instances
def instance_augment1(instances, question2targets):
    end_instances = []
    retain_keys = [key for key in instances[0].keys() if key != 'label']
    for instance in instances:
        question = instance["question"]
        targets = [instance["label"]]
        #targets = question2targets[question]
        label = instance["label"]
        targets_sen_labels = make_true_sen_labels_for_choose(
                targets, label)
...
def make_instances(samples,
    ...
    for sample in samples:
        ...
        def make_sentiment_polarity_labels1(str_):
            sentiment_polarity_labels = np.array([3] * len(str_), dtype=dtype)
            # 0：neutral, 1: positive 2: negative
            if "options" in sample.keys():
                for i in range(len(sample["options"])):
                    specify_senti = str(sample["options"][i])
                    if(specify_senti in str_):
                        if(sample['labels'][i]==0):
                            sentiment_polarity_labels[
                            str_.index(specify_senti): str_.index(specify_senti) + len(specify_senti)] = \
                                np.zeros(len(specify_senti), dtype=dtype)
                        elif(sample['labels'][i]==1):
                            sentiment_polarity_labels[
                            str_.index(specify_senti): str_.index(specify_senti) + len(specify_senti)] = \
                                np.ones(len(specify_senti), dtype=dtype)
                        elif(sample['labels'][i]==2):
                            sentiment_polarity_labels[
                            str_.index(specify_senti): str_.index(specify_senti) + len(specify_senti)] = \
                                np.ones(len(specify_senti), dtype=dtype)*2
            return sentiment_polarity_labels
        def make_sentiment_polarity_labels2(str_):
            sentiment_polarity_labels = np.array([3] * len(str_), dtype=dtype)
            # 0：neutral, 1: positive 2: negative
            if "options" in sample.keys():
                for i in range(len(sample["options"])):
                    if(str(sample["options"][i]) in str_):
                        specify_senti = str(sample["options"][i])
                        if(sample['labels'][i]==0):
                            sentiment_polarity_labels[
                            str_.index(specify_senti): str_.index(specify_senti) + len(specify_senti)] = \
        ...
        def make_indicate_target_labels():
            ...
            if "options" in sample.keys():
                for each in sample["options"]:
                    specify_target = str(each)
        ...
        if "answer" in sample.keys():
            ans_sentiment_polarity_labels = make_sentiment_polarity_labels2(ans)
        ...
        que_sentiment_polarity_labels = make_sentiment_polarity_labels1(que)
