    ...
    for batch_index in range(dev_data_stream.get_nb_batch()):  # for each batch
        ...
        if out_path is not None:
            for i in range(cur_batch.batch_size):
                if hasattr(cur_batch, 'target'):
                    current_line = "question:{}\tanswer:{}\ttarget:{}\ttruth:{}\t\n".format(
                        str(cur_batch.questions[i]), str(answers[i]),
                        str(cur_batch.targets[i]), str(cur_batch.truths[i]), )
                else:
                    current_line = "question:{}\tanswer:{}\ttruth:{}\tclass:{}\t\n".format(
                        str(cur_batch.questions[i]), str(answers[i]),
                        str(cur_batch.truths[i]),str(cur_batch.classes[i]) )
