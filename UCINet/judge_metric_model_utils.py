       ...
       # pred4=valid_graph.predictions4
        ...
        if out_path is not None:
            for i in range(cur_batch.batch_size):
                current_line1 = "que:{}\tans:{}\tlabel:{}\tpre_x{}\n".format(
                    str(cur_batch.questions[i]), str(cur_batch.answers[i]),
                    str(cur_batch.labels[i][0]),
                    str(prediction_ls_x[0][i]))
                current_line2 = "que:{}\tans:{}\tlabel:{}\tpre_x{}\n".format(
                    str(cur_batch.questions[i]), str(cur_batch_p.answers[i]),
                    str(cur_batch_p.labels[i][0]),
                    str(prediction_ls_xp[0][i]))
                current_line3 = "que:{}\tans:{}\tlabel:{}\tpre_x{}\n".format(
                    str(cur_batch_n.questions[i]), str(cur_batch_n.answers[i]),
                    str(cur_batch_n.labels[i][0]),
                    str(prediction_ls_xn[0][i]))
                outfile.writelines(current_line1)
                outfile.writelines(current_line2)
                outfile.writelines(current_line3)
