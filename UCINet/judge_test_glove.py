...
from judge_model import AnswerUnderstander
...
from compute_acc import acc
from result_copy import result_show


if __name__ == "__main__":
    with open('judge_config_glove.json', encoding='utf-8') as infile:
    ...
    with tf.Graph().as_default():
        ...
        evaluation(sess, answer_understander_dev,
                                                           data_stream_choose_dev, 'result_{}.txt'.format("wu_metric"))
        result_show('result_{}.txt'.format("wu_metric"),'result_{}.txt'.format("wu_metric_"))
        accuracy = acc('result_{}.txt'.format("wu_metric"))
        print(accuracy)
