...
def evaluation(sess, valid_graph, dev_data_stream, out_path=None):
    ...
    for batch_index in range(dev_data_stream.get_nb_batch()):  # for each batch
       ...
       # pred4=valid_graph.predictions4
        ...
        [cor,prediction_ls] = sess.run([valid_graph.acc,valid_graph.prediction_ls],feed_dict=feed_dict)
        correct+=cor
        if out_path is not None:

            for i in range(cur_batch.batch_size):
                current_line = "question:{}\tanswer:{}\toptions:{}\tlabel:{}\tprediction_ls{}\n".format(
                    str(cur_batch.questions[i]), str(cur_batch.answers[i]),
                    str(cur_batch.options[i]), str(cur_batch.labels[i]),
                    str(prediction_ls[0][i])+" "+str(prediction_ls[1][i])+" "+str(prediction_ls[2][i])+" "+str(prediction_ls[3][i])+" "+str(prediction_ls[4][i]))
                #current_line = "question:{}\tanswer:{}\toptions:{}\tlabel:{}\tprediction_ls{}\n".format(
                #    str(cur_batch.questions[i]), str(cur_batch.answers[i]),
                #    str(cur_batch.options[i]), str(cur_batch.labels[i][0]),
                #    str(prediction_ls[0][i]))
...
def train(nb_epoch, sess, saver, data_stream_train,
          data_stream_valid, answer_understander_train,
    ...
    for epoch in range(nb_epoch):
        ...
        acc= evaluation(sess, answer_understander_valid, data_stream_valid)
