...
def get_target_representation(in_value, indicate_matrix,t1,t2):
    # in_value: [batch_size, seq_length, feature_dim]
    # indicate_matrix: [batch_size, seq_length]
    feature_dim = in_value.get_shape().as_list()[-1]
    indicate_matrix = tf.to_float(
    ...
    target_repr = tf.reduce_sum(tf.multiply(in_value, indicate_matrix),
                                axis=1)
    target_repr = tf.multiply(target_repr, target_num)
    w_target = tf.get_variable(t1,
                               shape=[feature_dim, feature_dim],
                               dtype=tf.float32)
    b = tf.get_variable(t2, shape=[feature_dim], dtype=tf.float32)
...
def get_aware_repr(answer_repr, skeleton_repr, semantic_repr, nb_hops,
                   attention_dim, rnn_dim, ans_max_len, ans_lens, l2_reg,b1,b2):
...
def evaluation(sess, valid_graph, dev_data_stream_x,dev_data_stream_xp,dev_data_stream_xn, out_path=None):
    if out_path is not None:
        outfile = open(out_path, 'w', encoding='utf-8')
    total = 0
    correct=0
    num_batch_dev_x = dev_data_stream_x.get_nb_batch()
    num_batch_dev_xp = dev_data_stream_xp.get_nb_batch()
    num_batch_dev_xn = dev_data_stream_xn.get_nb_batch()
    num_batch_dev = min(num_batch_dev_x,num_batch_dev_xp,num_batch_dev_xn) 
    for batch_index in range(num_batch_dev):  # for each batch
        cur_batch = dev_data_stream_x.get_batch(batch_index)
        cur_batch_p = dev_data_stream_xp.get_batch(batch_index)
        cur_batch_n = dev_data_stream_xn.get_batch(batch_index)
        total += 16
        if valid_graph.use_bert:
            cur_batch.update_bert_vector()
            cur_batch_p.update_bert_vector()
            cur_batch_n.update_bert_vector()
        feed_dict = valid_graph.create_feed_dict(cur_batch,cur_batch_p,cur_batch_n,batch_index/num_batch_dev)
       # corr1=valid_graph.eval_correct1
       # corr2=valid_graph.eval_correct2
       # corr3=valid_graph.eval_correct3
       # corr4=valid_graph.eval_correct4
       # corr5=valid_graph.eval_correct5
       # pred1=valid_graph.predictions1
       # pred2=valid_graph.predictions2
       # pred3=valid_graph.predictions3
       # pred4=valid_graph.predictions4
        #pred5=valid_graph.predictions5
        #[cor] = sess.run([valid_graph.acc],feed_dict=feed_dict)
        [cor,prediction_ls_x,prediction_ls_xp,prediction_ls_xn] = sess.run([valid_graph.acc,valid_graph.prediction_ls_x,valid_graph.prediction_ls_xp,valid_graph.prediction_ls_xn],feed_dict=feed_dict)
        correct+=cor
        if out_path is not None:
            for i in range(cur_batch.batch_size):
                current_line = "que:{}\tans1:{}\toption:{}\tlabel1:{}\tpre_x{}\tans2:{}\tlabel2:{}\tpre_xp:{}\tans3:{}\tlabel3:{}\tpre_xn:{}\n".format(
                    str(cur_batch.questions[i]), str(cur_batch.answers[i]),
                    str(cur_batch.options[i]), str(cur_batch.labels[i]),
                    str(prediction_ls_x[0][i])+" "+str(prediction_ls_x[1][i])+" "+str(prediction_ls_x[2][i])+" "+str(prediction_ls_x[3][i])+" "+str(prediction_ls_x[4][i]),
             str(cur_batch_p.answers[i]),str(cur_batch_p.labels[i]), str(prediction_ls_xp[0][i])+" "+str(prediction_ls_xp[1][i])+" "+str(prediction_ls_xp[2][i])+" "+str(prediction_ls_xp[3][i])+" "+str(prediction_ls_xp[4][i]),
             str(cur_batch_n.answers[i]),str(cur_batch_n.labels[i]), str(prediction_ls_xn[0][i])+" "+str(prediction_ls_xn[1][i])+" "+str(prediction_ls_xn[2][i])+" "+str(prediction_ls_xn[3][i])+" "+str(prediction_ls_xn[4][i])
             )
                outfile.writelines(current_line)
                #predict_result.append(predictions[i])
                
    acc=correct/float(total) * 100
    print(acc)
        #for i in range(5):
         #   name_cur_corre="cur_correct"+str(i+1)
          #  name_pre = "predictions"+str(i+1)
           # name_eval_correct = "eval_correct"+str(i+1)
            #name_corre = "correct"+str(i+1)
            #[locals()["cur_correct"+str(i+1)],locals()["predictions"+str(i+1)]] = sess.run([locals()["corr"+str(i+1)],locals()["pred"+str(i+1)]],feed_dict=feed_dict)
          #  locals()["correct"+str(i+1)]+=locals()["cur_correct"+str(i+1)]


    #if out_path is not None:
     #   outfile.writelines("the final accuracy:{}".format(accuracy1))
      #  outfile.close()
    return acc


def train(nb_epoch, sess, saver, data_stream_train_x,data_stream_train_xp,data_stream_train_xn,
          data_stream_valid_x,data_stream_valid_xp,data_stream_valid_xn, answer_understander_train,
          answer_understander_valid, best_path):
    best_accuracy = -1
    #for i in range(5):
    #    name_corre = "correct"+str(i+1)
    #    locals()["correct"+str(i+1)] =0
    for epoch in range(nb_epoch):
        print('Train in epoch %d' % epoch)
        num_batch_x = data_stream_train_x.get_nb_batch()
        num_batch_xp = data_stream_train_xp.get_nb_batch()
        num_batch_xn = data_stream_train_xn.get_nb_batch()
        num_batch = min(num_batch_x,num_batch_xp,num_batch_xn)
        total_loss = 0
        # training
        #data_stream_train.shuffle()
        for batch_index in tqdm(range(num_batch)):
            cur_batch = data_stream_train_x.get_batch(batch_index)
            cur_batch_p = data_stream_train_xp.get_batch(batch_index)
            cur_batch_n = data_stream_train_xn.get_batch(batch_index)
            if answer_understander_train.use_bert:
                cur_batch.update_bert_vector()
                cur_batch_p.update_bert_vector()
                cur_batch_n.update_bert_vector()
            feed_dict = answer_understander_train.create_feed_dict(cur_batch,cur_batch_p,cur_batch_n,batch_index/num_batch)
            loss, _, = sess.run([
        ...
        acc= evaluation(sess, answer_understander_valid, data_stream_valid_x, data_stream_valid_xp, data_stream_valid_xn)
        #duration = time.time() - start_time
        #print(epoch, total_loss / num_batch)
